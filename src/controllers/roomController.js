...
function inferFloorId(roomId = "") {
  ...
  if (digitMatch) {
    return `Floor ${digitMatch[1]}`;
  }
  const prefixMatch = text.match(/^[A-Z]+/);
  return prefixMatch ? prefixMatch[0] : "Other";
}
...
async function getAvailableFloors(req, res) {
  try {
    ...
    const floors = Array.from(new Set(roomIds.filter(Boolean).map(inferFloorId))).sort();
    res.json({ floors });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
async function getAvailableRooms(req, res) {
  try {
    ...
    let rooms = (await SensorReading.distinct("room_id")).filter(Boolean).sort();
